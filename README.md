...
3.  一个基于 **穷举法的选择题自动提交脚本 (`bruteforce_solver.py`)**，运行后可进行命令行交互，设置关卡题目属性，可以自动调用submit.py提交，每次提交后根据使用者反馈的题目出错情况自动排除错误选项，最多4次尝试（小于每个关卡每天最多提交的5次）即可穷举过关。灵感来源于[LMQ](https://github.com/Lmq0)
